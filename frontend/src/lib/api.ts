...
class ApiClient {
  ...
  private normalizeProject(project: any): Project {
    ...
    if (Array.isArray(project.tech_stack)) {
      flatTech = project.tech_stack.filter((item: any): item is string => typeof item === "string");
  ...
  async fetchApi<T>(
    ...
    // Use mock client ONLY when explicitly flagged offline in dev environments (localhost/tunnels)
    ...
    try {
      ...
      // Otherwise, it is a network connection error. Fallback to mock only in dev
      if (typeof window !== "undefined" && 
          (window.location.hostname === "localhost" || 
           window.location.hostname === "127.0.0.1" || 
           window.location.hostname.includes("loca.lt") || 
